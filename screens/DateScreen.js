        ...
        >
          {this.state.dates == undefined ? (
            ...
            <MainScreen
              ...
              apiProps={this.props.route.params}
...
function datesListItem(item, date, setDate) {
  return (
    <TouchableOpacity
      style={item == date ? styles.hItemWrapperSelected : styles.hItemWrapper}
      onPress={() => {
        setDate(item);
      }}
    >
      <Text style={item == date ? styles.hItemSelected : styles.hItem}>
        {new Date(Date.parse(item)).toLocaleDateString("rus")}
      </Text>
    </TouchableOpacity>
  );
}
    ...
    return (
      <View>
        {this.state.times == undefined
          ? loadingScreen()
          : timesList(this.state.times, this.props.navigation, apiProps)}
      </View>
    );
  }
}
