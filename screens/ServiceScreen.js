...
class ServiceScreen extends React.Component {
  constructor(props) {
    ...
    api.getServices(
      (result) => {
        ...
        nextState.renderedServices = result.filter(
          (e) => e.category_id == 8279741
        );
  ...
  setPro(pro) {
    ...
    state.pro = pro;
    if (pro) {
      state.renderedServices = state.services.filter(
        (e) => e.category_id == 8279741
      );
    } else {
      state.renderedServices = state.services.filter(
  ...
  render() {
    return (
      ...
      >
        <SafeAreaView
          style={[styles.container, { paddingTop: this.props.insets.top + 8 }]}
        >
          {this.state.services == undefined
            ? loadingScreen()
            : servicesScreen(
                this.state.renderedServices,
  ...
  return (
    <View>
      <View style={styles.hlist}>
        <TouchableOpacity
          style={pro ? styles.hItemWrapperSelected : styles.hItemWrapper}
          onPress={() => {
            setPro(true);
          }}
        >
          <Text style={pro ? styles.hItemSelected : styles.hItem}>
            Услуги PRO
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={pro ? styles.hItemWrapper : styles.hItemWrapperSelected}
          onPress={() => {
            setPro(false);
          }}
        >
          <Text style={pro ? styles.hItem : styles.hItemSelected}>
            Услуги Барбершопа
          </Text>
        </TouchableOpacity>
      </View>
...
function serviceListItem(item, navigation) {
  return (
    <TouchableOpacity
      style={styles.item}
      onPress={() => {
        navigation.navigate("Staff", { service: item });
      }}
    >
